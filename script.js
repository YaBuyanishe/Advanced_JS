...
const changeModalState = state => {
  const windowForm = document.querySelectorAll('.balcon_icons_img'),
        ...
        windowProfile = document.querySelectorAll('.checkbox'); // Деактивируем кнопки в калькуляторе
      ...
      item.addEventListener(event, () => {
        switch (item.nodeName) {
          ...
          case 'SELECT':
            state[prop] = item.value;
            break;
        } // Активируем кнопки в калькуляторе при наличии нужных значений
...
const forms = state => {
  // Получаем переменные для работы с формами
  const form = document.querySelectorAll('form'),
        input = document.querySelectorAll('input'); // Проверка на ввод цифр в поле номера телефона

  (0,_checkNumInput__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user_phone"]'); // Создаём объект для показа пользователю

  const message = {
    loading: 'Загрузка',
    success: 'Спасибо! Скоро мы свяжемся с Вами!',
    failure: 'Что-то пошло не так...'
  }; // Функция для отправки запросов, через использование промисов
  ...
  form.forEach(item => {
    // Обработчик собитый при нажатии кнопки
    item.addEventListener('submit', e => {
      e.preventDefault(); // Показ пользователю сообщения после нажатия кнопки
...
const modals = () => {
  // Функция отвечающая за вызов модалки
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    const trigger = document.querySelectorAll(triggerSelector),
          // все селекторы, что бы повесить одинаковые функции на несколько селекторов
    modal = document.querySelector(modalSelector),
          // селектор модалки
    close = document.querySelector(closeSelector),
          // селектор закрытия модалки
...
const tabs = function (headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', e => {
    // Обработчик событий на области, которая объединяет все табы
    const target = e.target;

    if (target && (target.classList.contains(tabSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      // Проверяем, что кликнули в один из табов, а не в пустое место или элмент внутри таба
      tab.forEach((item, i) => {
        // Перебираем табы один за другим,запоминаем не только перебираемый таб но и его номер по порядку.
        if (target == item || target.parentNode == item) {
          // Если выполняется условие, что перебираемый таб равен табу, на который кликнули, запоминаем i и к нему применяется действие функции.
          hideTabContent();
          showTabContent(i);
        }
      });
...
/* harmony import */ var slick_carousel__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(slick_carousel__WEBPACK_IMPORTED_MODULE_1__);
 // импорт $ из jQuery // npm i jquery --save
...
/* harmony import */ var _modules_images__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./modules/images */ "./src/js/modules/images.js");
 // установелен через npm i slick-carousel --save

 // подключение модуля в основной файл js.
...
window.addEventListener('DOMContentLoaded', () => {
  'use strict'; // Переменная для отображения состояния модального окна

  let modalState = {
    form: 0,
    type: 'tree'
  };
  let deadline = '2022-05-31';
